...
### 3.59  那如果客户端不发送数据，什么时候才会断开处于 ESTABLISHED 状态的连接？
...
### 3.61 在 Linux 上如何打开 Fast Open 功能？
...
### 3.65  在 Wireshark 看到的 Windows size 也就是 " win = "，这个值表示发送窗⼝吗？
...
### 3.70 Nagle 算法是如何避免⼤量 TCP ⼩数据报⽂的传输？
...
### 3.72 延迟确认 和 Nagle 算法混合使⽤时，会产⽣新的问题
...
### 3.73 tcp_retries1 参数，是什么场景下⽣效？tcp_retries2是不是只受限于规定的次数，还是受限于次数和时间限制的最⼩值？”
...
### 3.74 tcp_orphan_retries也是控制tcp连接的关闭。这个跟tcp_retries1 tcp_retries2有什么区别吗？
...
### 3.75 请问，为什么连续两个报⽂的seq会是⼀样的呢，⽐如三次握⼿之后的那个报⽂？还是说，序号相同的是同⼀个报⽂，只是拆开显示了？
...
### 3.77 在 TCP 三次握⼿的时候，Linux 内核会维护两个队列，分别是：
...
### 3.78 实战 - TCP 全连接队列溢出之如何知道应⽤程序的 TCP 全连接队列⼤⼩？
...
### 3.80 Linux 有个参数可以指定当 TCP 全连接队列满了会使⽤什么策略来回应客户端。
...
### 3.83 实战 - TCP 半连接队列溢出之如何查看 TCP 半连接队列⻓度？
...
### 3.85 ⼤部分⼈都说 tcp_max_syn_backlog 是指定半连接队列的⼤⼩，是真的吗？
...
### 3.86 半连接队列最⼤值 max_qlen_log 就表示服务端处于 SYN_REVC 状态的最⼤个数吗？
...
### 3.87 每个 Linux 内核版本「理论」半连接最⼤值计算⽅式会不同。
...
### 3.90 读者问：“syncookies 启⽤后就不需要半链接了？那请求的数据会存在哪⾥？”
...
### 3.93 如何查看由于 SYN 半连接队列已满，⽽被丢弃连接的情况？
...
### 3.100 如何查看由于 accept 连接队列已满，⽽被丢弃的连接？
...
### 3.103 Linux 下怎么打开 TCP Fast Open 功能呢？
...
### 3.111 如果 accept 队列满了，那么 server 扔掉 client 发过来的 ack”，也就是说该TCP连接还是位于半连接队列中，没有丢弃吗？”
