...
### 3.55 为什么抓到的 TCP 挥⼿是三次，⽽不是书上说的四次？
...
### 3.57 咦？客户端设置了防⽕墙，屏蔽了服务端的⽹络包，为什么 tcpdump 还能抓到服务端的⽹络包？
...
### 3.58 tcp_syn_retries 是限制 SYN 重传次数，那第⼆次握⼿ SYN、ACK 限制最⼤重传次数是多少？
...
### 3.59  TCP 第⼀次握⼿的 SYN 包超时重传最⼤次数是由 tcp_syn_retries 指定，TCP 第⼆次握⼿的 SYN、ACK 包超时重传最⼤次数是由 tcp_synack_retries 指定，那 TCP 建⽴连接后的数据包最⼤超时重传次数是由什么参数指定呢？
...
### 3.60  那如果客户端不发送数据，什么时候才会断开处于 ESTABLISHED 状态的连接？
...
### 3.62 在 Linux 上如何打开 Fast Open 功能？
...
### 3.66  在 Wireshark 看到的 Windows size 也就是 " win = "，这个值表示发送窗⼝吗？
...
### 3.71 Nagle 算法是如何避免⼤量 TCP ⼩数据报⽂的传输？
...
### 3.73 延迟确认 和 Nagle 算法混合使⽤时，会产⽣新的问题
...
### 3.74 tcp_retries1 参数，是什么场景下⽣效？tcp_retries2是不是只受限于规定的次数，还是受限于次数和时间限制的最⼩值？”
...
### 3.75 tcp_orphan_retries也是控制tcp连接的关闭。这个跟tcp_retries1 tcp_retries2有什么区别吗？
...
### 3.76 请问，为什么连续两个报⽂的seq会是⼀样的呢，⽐如三次握⼿之后的那个报⽂？还是说，序号相同的是同⼀个报⽂，只是拆开显示了？
...
### 3.78 在 TCP 三次握⼿的时候，Linux 内核会维护两个队列，分别是：
...
### 3.79 实战 - TCP 全连接队列溢出之如何知道应⽤程序的 TCP 全连接队列⼤⼩？
...
### 3.81 Linux 有个参数可以指定当 TCP 全连接队列满了会使⽤什么策略来回应客户端。
...
### 3.84 实战 - TCP 半连接队列溢出之如何查看 TCP 半连接队列⻓度？
...
### 3.86 ⼤部分⼈都说 tcp_max_syn_backlog 是指定半连接队列的⼤⼩，是真的吗？
...
### 3.87 半连接队列最⼤值 max_qlen_log 就表示服务端处于 SYN_REVC 状态的最⼤个数吗？
...
### 3.88 每个 Linux 内核版本「理论」半连接最⼤值计算⽅式会不同。
...
### 3.91 读者问：“syncookies 启⽤后就不需要半链接了？那请求的数据会存在哪⾥？”
...
### 3.94 如何查看由于 SYN 半连接队列已满，⽽被丢弃连接的情况？
...
### 3.101 如何查看由于 accept 连接队列已满，⽽被丢弃的连接？
...
### 3.104 Linux 下怎么打开 TCP Fast Open 功能呢？
...
### 3.112 如果 accept 队列满了，那么 server 扔掉 client 发过来的 ack”，也就是说该TCP连接还是位于半连接队列中，没有丢弃吗？”
